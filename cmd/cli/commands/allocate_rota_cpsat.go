...
// AllocateRotaCpsatCmd creates the allocateRotaCpsat command
func AllocateRotaCpsatCmd(app *AppContext) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			app.Logger.Debug("allocateRotaCpsat command",
				zap.Bool("dry_run", dryRun),
				zap.Bool("force_commit", forceCommit),
				zap.String("python", pythonFlag))

			result, err := services.AllocateRotaCpsat(
				app.Ctx,
				app.Database,
				app.SheetsClient,
				app.FormsClient,
			...
			if !result.Success {
				...
				fmt.Println("   Unlike the greedy allocator, CP-SAT will not produce a rule-breaking rota.")
