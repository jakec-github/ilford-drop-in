...
# Neither the jobs volunteers hold, nor when the drop-in runs, nor what a shift
# asks for are configured here. Roles and Rota Defaults — the default shift
# start, end and timezone, and the default Shape — are rows in the database
# (ADR 0006), set on the Settings screen rather than in this file. A `roles:`,
# `shiftStartTime:`, `shiftEndTime:`, `shiftTimezone:`, `defaultShiftSize:`,
# `shiftSize:` or `preallocations:` key left over from an older config is
