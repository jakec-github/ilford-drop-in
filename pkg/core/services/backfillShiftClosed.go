...
//
// It is a one-off, run once against each environment from the commit that
// carries it, and deleted in the next one along with the config key it reads.
// Every Shift is minted open, so the
