...
// AvailabilityRequestResult represents volunteers who need availability requests
type AvailabilityRequestResult struct {
	LatestRota               *db.Rotation
	VolunteersWithoutRequest []model.Volunteer
}
...
// RequestAvailability identifies volunteers who need availability forms for the latest rota
// It fetches the latest rota, existing availability requests, and active volunteers
// Returns volunteers who don't have requests yet
func RequestAvailability(
	...
	allVolunteers, err := client.ListVolunteers(cfg)
	if err != nil {
	...
	return &AvailabilityRequestResult{
		...
		VolunteersWithoutRequest: volunteersWithoutRequests,
	}, nil
}
