...
func TestRequestAvailability_CreatesRequestsForVolunteersWithoutRequests(t *testing.T) {
	...
	mockVolunteerClient := &mockVolunteerClient{
		volunteers: []model.Volunteer{
			{ID: "vol-1", FirstName: "John", LastName: "Doe", Status: "Active"},
			{ID: "vol-2", FirstName: "Jane", LastName: "Smith", Status: "Active"},
			{ID: "vol-3", FirstName: "Bob", LastName: "Jones", Status: "Active"},
		},
	}
	...
	result, err := RequestAvailability(ctx, mockStore, mockVolunteerClient, mockFormsClient, cfg, logger, "2024-01-15")

	require.NoError(t, err)
	require.NotNil(t, result)

	// Should have created 2 new requests (vol-2 and vol-3)
	require.Len(t, mockStore.insertedRequests, 2)

	// Verify inserted requests
	insertedIDs := make(map[string]bool)
	for _, req := range mockStore.insertedRequests {
		insertedIDs[req.VolunteerID] = true
	...
	assert.True(t, insertedIDs["vol-2"], "Should have created request for vol-2")
	assert.True(t, insertedIDs["vol-3"], "Should have created request for vol-3")
	assert.False(t, insertedIDs["vol-1"], "Should not have created request for vol-1 (already exists)")
...
func TestRequestAvailability_OnlyCreatesForLatestRota(t *testing.T) {
	...
	// Should create request for vol-1 for the latest rota
